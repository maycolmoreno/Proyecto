...
**Última actualización:** 2026-07-10 (remoción de n8n, post-cierre)
...
| 2026-07-09 | **Extensión post-cierre (Sprint F5 frontend, `docs/PLAN_FRONTEND.md`):** gap real detectado al construir Mensajería — Fase 4 nunca definió una forma de resolver el *nombre* de otro usuario a partir de su id; `GET /usuarios/me` es solo el perfil propio y `GET /admin/usuarios` (Sprint F4) exige rol ADMINISTRADOR. Sin esto, la lista de conversaciones (`GET /conversaciones`, que solo devuelve ids de `participantes`) y "Enviar mensaje al publicador" no podían mostrar más que un UUID crudo. Confirmado explícitamente con el usuario antes de construir (tercera vez que se sigue este criterio, tras Entrega en F2 y Usuarios-admin en F4). Se agrega `ObtenerUsuarioPublicoUseCase` en `application/identidad/` — reutiliza `UsuarioNoEncontradoError` ya exportado por `ObtenerPerfilUseCase.ts` en vez de duplicarlo (mismo criterio que `TruequeNoEncontradoError` compartido entre casos de uso de trueques) — y devuelve deliberadamente solo `{id, nombre}`, nunca correo/teléfono/rol/estado: a diferencia de `/usuarios/me` (dueño) y `/admin/usuarios` (ADMINISTRADOR), este endpoint es alcanzable por *cualquier* autenticado sobre *cualquier* otro usuario, así que el contrato es intencionalmente mínimo para no filtrar PII. `UsuariosController.obtenerPorId` + `GET /usuarios/:id` declarada **después** de `/usuarios/me` en `identidad.routes.ts` (si se declarara antes, `:id` capturaría el literal "me" — mismo problema de orden ya documentado para `entregas.routes.ts` en F2). Verificado con curl: `200` con `{id,nombre}` real consultando a otro usuario, `404` para un id inexistente, `/usuarios/me` sin regresión. |
| 2026-07-10 | **Remoción de n8n (ADR-047).** Decisión explícita del usuario tras probar el frontend: en vez de completar la configuración manual del workflow de n8n (llegó a evaluar Mailtrap y Gmail con contraseña de aplicación para las credenciales SMTP), pidió sacar n8n del proyecto por completo. Se eliminan `adapters/notificaciones/external/N8nWebhookAdapter.ts`, `domain/notificaciones/ports/IWebhookNotifier.ts`, `adapters/notificaciones/repositories/MongooseLogsN8nRepository.ts`, `domain/notificaciones/ports/ILogsN8nRepository.ts` y el servicio `n8n` + volumen `n8n_data` de `docker-compose.yml`. `NotificacionDispatchService` pierde el parámetro `webhookNotifier` de su constructor; `notificarConCorreo` se colapsa en `notificar` (ya no hay distinción "7 eventos de alto valor" — Fase 8 sección 5 queda sin efecto, ver `docs/fases/fase-08-automatizaciones.md`, marcada como removida). `env.ts` pierde `N8N_WEBHOOK_URL`. Sin cambios en el resto del catálogo de eventos ni en `INotificacionRepository`/`entidadTipo` (extensión de Sprint F5, sigue vigente — el frontend sigue navegando desde notificaciones igual que antes). Verificado: `npm run typecheck && npm run lint` limpios, contenedor `api` recreado sin `N8N_WEBHOOK_URL`, contenedor y volumen de `n8n` detenidos y eliminados, flujo de notificación in-app (`DonacionPublicada`) probado con curl real después de la remoción — se sigue creando correctamente con `entidadTipo`. Detalle completo en `docs/DECISIONES.md` ADR-047. |
