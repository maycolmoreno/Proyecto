...
**Última actualización:** 2026-07-08 (Sprint 5 — cambio de proveedor)
...
| 2026-07-08 | Sprint 4 (implementación, `docs/fases/fase-06-backend.md`): dos desviaciones respecto al diseño de esta fase, documentadas por trazabilidad. (1) Sección 3 dice que la clasificación "se registra en `analisis_ia` junto con el score/confianza" — en la práctica el disparo ocurre *antes* de publicar (mismo párrafo: "al llegar al último paso del wizard"), por lo que no existe un `entidadId` real todavía; se decidió **no persistir** el resultado de clasificación en Mongo en vez de introducir un id temporal fuera de alcance. (2) Sección 4 menciona `MapsAdapter.calcularDistancia` para el filtro determinista de matching — este componente **no existe** en el proyecto (ningún flujo de Sprints 1-3 filtra por radio geográfico, solo por provincia/ciudad exacta); el filtro determinista de matching implementado usa el mismo mecanismo de categoría+estado(+provincia/ciudad cuando aplica) que ya usan los listados existentes, sin agregar geodistancia. Ninguna de las dos desviaciones afecta el enfoque híbrido (filtro determinista + scoring IA) ni los modelos/prompts/salida estructurada, que se implementaron tal como se diseñaron aquí. |
| 2026-07-08 | **Cambio de proveedor de IA — desvía ADR-024** (sección 1: "Se elige Claude (Anthropic)"). La `IA_API_KEY` de Anthropic provista en Sprint 4 resultó inválida (`401 invalid x-api-key`, ver historial de `fase-06-backend.md`), y Anthropic no ofrece un tier gratuito permanente — solo créditos limitados para cuentas nuevas. El usuario solicitó explícitamente cambiar a **Gemini (Google)**, que sí ofrece API keys gratuitas sin tarjeta vía Google AI Studio. Se agrega `adapters/ia/external/GeminiAdapter.ts` (`@google/genai`, SDK oficial) implementando el mismo puerto `IIAProvider` sin ningún cambio en `domain/`/`application/` — la arquitectura hexagonal ya anticipaba esta sustitución (Fase 2 sección 2: "oculta el proveedor concreto detrás de una interfaz de dominio"). `ClaudeAdapter` **no se elimina**, queda disponible como alternativa si se consigue una key de Anthropic válida; `main/di-container.ts` cablea uno u otro con un cambio de una línea. Modelos elegidos siguiendo el mismo criterio de diferenciación por tarea que Fase 7 sección 1: `gemini-3.5-flash` para el chatbot (mejor calidad conversacional dentro del tier Flash, análogo al rol de Sonnet), `gemini-2.5-flash-lite` para clasificar/matchScore/evaluarRiesgo (más barato/rápido, tareas acotadas de alto volumen, análogo al rol de Haiku). Salida estructurada vía `responseSchema`/`responseMimeType: "application/json"` de Gemini reemplaza a `output_config.format` de Anthropic — mismo enfoque de diseño (ADR-025 sigue vigente: salida estructurada, nunca parsing de texto libre), solo cambia el mecanismo concreto del SDK. Prompt caching (ADR-026) **no tiene equivalente directo verificado en Gemini** en esta implementación — no se investigó ni se implementó; queda como posible desviación de costo a revisar si el volumen de uso lo justifica. **Verificado end-to-end contra la API real de Gemini (2026-07-08), los 4 flujos:** chatbot (respuesta coherente en español, ~5s, dentro de RNF-002 ≤10s), `POST /ia/clasificar` (categoría/título/descripción sugeridos correctamente vía `responseSchema`), `GET /ia/matching` (score 0.8 con razón coherente), y `ModeracionIAService` disparada automáticamente al publicar una donación (`riesgoDetectado: false`, `confianza: 0.9`, registrado en `analisis_ia`). Se observó un `503 UNAVAILABLE` transitorio de `gemini-2.5-flash-lite` en el primer intento de clasificar ("alta demanda, intenta más tarde") — típico del tier gratuito bajo carga, no un error de código; reintentar resolvió. Sin acciones pendientes de credenciales. |
